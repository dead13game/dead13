...
> - 不能（需要读代码 / 算法 / 数据）→ **AI 做**
>
> **例外（旧规则已废除）**：**初步 UI 布局设计归 AI**——新 UI 区块/按钮/面板的
> 初始位置、尺寸、间距由 AI 按审美给一版合理布局（遵循布局准则：绝对定位/容器分流/
> 最小字号 26px），人类只做**最终微调与手感验收**。微调走 F1 调试浮层 + UI 调参子代理
> （报「节点名+效果」，不翻代码），见 `tools/ui_adjust_agent.md`、`docs/ui-adjust-workflow.md`。

视觉类任务让 AI 做是「盲调参数 → 你截图 → 再调」的死循环，一次改动烧几十轮
调用；人类在 Godot 编辑器里所见即所得，30 秒搞定。**这一条对「最终微调」依然成立，
只把「出初版」交给了 AI。**
...
2. **批量攒单**：别一条条来——把一周的「需要 AI 的活」攒成一单一次性给 AI，
   减少上下文反复加载
3. **AI 产出骨架/接口 + 初步 UI 布局**：约定好接口签名（如
   `damage_number(text, pos, color)`），AI 写一次；新 UI 区块的初始位置/尺寸
   由 AI 按审美给初版（旧规则「AI 不产出成品」中布局部分已废除），人类只微调
4. **视觉手感微调零 AI 试错**：最终位置/动画/粒子/音效的手感微调与验收默认人类做
   （AI 不自己开游戏试错）；微调 UI 走 F1 调试浮层 + UI 调参子代理，报「节点名+效果」
   一句话搞定，不翻代码、不烧 token
