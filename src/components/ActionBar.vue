  ...
  <div class="action-bar">
    ...
    <template v-if="state.step === 'pickAction'">
      <div class="action-row">
        <button
          class="ab ab--atk"
          :disabled="disabled || state.phase === 'peace'"
          @click="$emit('attack')"
        >
          攻击
        </button>
    ...
    <template v-else-if="state.step === 'attackShowCard'">
      <div class="attack-show">
        ...
        <div class="attack-show__cards">
          ...
          <Card
            v-else-if="pendingAttackCard"
            :key="'atk-' + pendingAttackCard.id"
        ...
        <div class="attack-show__total" v-if="ventiCards">
          <span class="action-hint"
            >千风之诗 合计点数：{{ pendingAttackCard?.value || 0 }}</span
          >
        </div>
    ...
    <template v-else-if="state.step === 'skillPickTarget'">
      <span class="action-hint">{{
        state.pendingFurinaTarget
          ? "选择要交换陷阱的目标："
          : state._aimiliyaFreeze
            ? "选择要冻结的目标："
            : "选择技能目标："
      }}</span>
      <div class="action-row">
        <button
          ...
          :disabled="disabled"
          @click="$emit('skillTarget', p.index)"
        >
          {{ p.name }}
        </button>
        <button
    ...
    <template v-else-if="state.step === 'allyPick'">
      ...
      <div class="action-row">
        <button
          v-for="p in allianceTargets"
          :key="p.index"
          class="ab ab--ally"
          :disabled="disabled"
          @click="$emit('allyInvite', p.index)"
        >
          {{ p.name }}
        </button>
        <button
    ...
    <!-- 赌命选牌（AI 行动时隐藏） -->
    <template v-else-if="state.step === 'gamblePick' && !state._skipAnim">
    ...
    <template v-else-if="state.step === 'skillNahida'">
      <div class="scry-area">
        ...
        <div class="action-row">
          <button
            class="ab ab--confirm"
            :disabled="disabled || nahidaOrder.length < nahidaTotalCards"
            @click="onNahidaConfirm"
          >
            确定顺序
          </button>
          <button
...
function onGambleConfirm() {
  if (
    gambleTrapIdx.value >= 0 &&
    gambleBaitIdx.value >= 0 &&
    gambleTrapIdx.value !== gambleBaitIdx.value
  ) {
...
.tiny-btn--trap.active {
  background: #fb8c00;
  border-color: #fb8c00;
}
.tiny-btn--bait.active {
  background: #43a047;
  border-color: #43a047;
}
