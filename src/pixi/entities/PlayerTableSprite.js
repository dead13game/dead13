...
export class PlayerTableSprite extends Container {
  ...
  _build() {
    ...
    this.nameText = new Text({
      ...
      style: { fontSize: fsName, fontWeight: 'bold', fill: 0x333333, fontFamily: 'sans-serif' }
    })
    ...
    this.statusText = new Text({
      text: '',
      style: { fontSize: fsStatus, fill: 0xe65100, fontWeight: 'bold', fontFamily: 'sans-serif' }
    })
    ...
    this.hpText = new Text({
      text: '',
      style: { fontSize: fsHp, fill: 0x333333, fontWeight: 'bold', fontFamily: 'sans-serif' }
    })
    ...
    this.deadMark = new Text({
      text: '阵亡',
      style: { fontSize: compact ? 20 : 28, fill: 0xc62828, fontWeight: 'bold', fontFamily: 'sans-serif' }
    })
    ...
    if (portraitW > 8 && portraitH > 8) {
      this._portraitRect = { x: portraitX, y: portraitY, w: portraitW, h: portraitH }
      this._loadPortrait()
    }
  ...
  _loadPortrait() {
    ...
    if (!path) { this._drawPortraitPlaceholder(r); return }

    const img = new Image()
    img.src = path
    const setup = () => {
      if (!img.width) { this._drawPortraitPlaceholder(r); return }
  ...
  _syncDefenses(defensePile) {
    ...
    const areaH = this._layout.playerTableSize.height - this.defContainer.y - 10
