...
function checkGameOver(state) {
  ...
  if (alive.length <= 1) {
    // 比赛模式：不结束游戏，交给 matchContext 处理进球/重置
    if (state.matchContext) {
      // 防止 applyDamage 和 nextPlayer 各调一次导致比分+2
...
export function executeGamble(state) {
  ...
  addLog(
    state,
    `${player.name} 执行赌命 摸出${drawn.map(cardDisplay).join(" ")}`,
  );
...
export function executeSkill(state) {
  ...
  switch (player.characterId) {
    ...
    case "raiden":
      return executeSkillRaiden(state);
    case "nahida":
...
function restoreState(state, sp) {
  ...
  sp.players.forEach((sp, i) => {
    ...
    state.players[i].stealTarget = sp.stealTarget
      ? { ...sp.stealTarget }
      : null;
