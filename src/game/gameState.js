...
function createPlayer(index, charData, name) {
  return {
    ...
    // UI 显示用（+1行动标签），回合控制已改用 state.endTurn
    extraAction: false,
    // 菜月昴：读档次数
    loadUses: 3,
    loadMaxUses: 3,
...
/** 解除双方的联盟关系 */
function dissolveAlliance(state, player) {
...
export function initGame(state, playerChars, useWeather = false) {
  ...
  addLog(state, `角色：${state.players.map(p => p.characterName).join(' · ')}`)
  addLog(state, `行动顺序：${state.players.map(p => p.name).join(' → ')}`)
  addLog(state, `和平发育（第1-2回合禁止攻击）`)
}
...
function nextPlayer(state, _depth = 0) {
  ...
  if (next >= state.players.length || !state.players[next]?.alive) {
    ...
    if (state.useWeather) {
      drawWeather(state)
      addLog(state, `${WDATA[state.currentWeather]?.name || '风和日丽'}`)
...
function applyDamage(state, player, damage) {
  ...
  while (remaining > 0 && player.defensePile.length > 0) {
    ...
    if (top.value >= remaining) {
      ...
      if (top.value === 0) {
        ...
        addLog(state, `${player.name} 防御牌抵消`)
      } else {
        addLog(state, `${player.name} 残盾 ${top.value}点`)
      }
    } else {
      remaining -= top.value
      addLog(state, `${player.name} 防御牌 ${cardDisplay(top)} 被击穿`)
      player.defensePile.pop()
      if (!top.isShield) state.grave.push(top)
    }
  }
  ...
  if (remaining > 0) {
    player.hp -= remaining
    addLog(state, `${player.name} HP ${player.hp}`)

    if (player.hp <= 0) {
      player.alive = false
      player.hp = 0
      addLog(state, `${player.name} 阵亡`)
...
export function executeAttack(state, targetIdx) {
  ...
  if (state.pendingVentiCards) {
    ...
    state.pendingVentiCards = null
    addLog(state, `${attacker.name} 千风之诗 ${cardDisplay(attackCards[0])} ${cardDisplay(attackCards[1])}`)
  } else {
    attackCards = [card]
  ...
  if (attacker.characterId === 'liniya' && attacker.damageBonus[targetIdx] > 0) {
    ...
    addLog(state, `永久伤害+${attacker.damageBonus[targetIdx]}`)
  }
  ...
  if (attacker.characterId === 'fenjin') {
    ...
    if (healCount > 0) {
      ...
      addLog(state, `${attacker.name} 风堇被动回复 ${healCount} 点（当前 HP ${attacker.hp}）`)
    }
  }
...
export function executeGamble(state) {
  ...
  addLog(state, `${player.name} 执行赌命 摸出${drawn.map(cardDisplay).join(' ')}`)
...
function startSkillVenti(state) {
  ...
  state.step = STEP.ATTACK_SHOW_CARD
  player.skillUses--
  addLog(state, `${player.name} 千风之诗 ${cardDisplay(cards[0])} ${cardDisplay(cards[1])}`)
...
export function executeFenjinSkill(state, targetIdx) {
  ...
  addLog(state, `${player.name} 生命上限 ${oldMaxHp}→${player.maxHp}，回复 ${healAmount} 点`)
...
export function executeLiniyaSkill(state, targetIdx, subSkill) {
  ...
  if (subSkill === 1) {
    ...
    addLog(state, `${player.name} 偷取 ${target.name} 的防御牌（3回合），对其伤害+2`)
  } else {
    // 技能二：每回合5点DoT伤害无视陷阱持续5回合
    player.dotTarget = { idx: targetIdx, turns: 5 }
    addLog(state, `${player.name} 对 ${target.name} 施加5回合DoT（每回合5点无视陷阱）`)
  }
...
export function executeCaiyueangLoad(state) {
  ...
  addLog(state, `${player.name} 死亡回归！回溯到存档点（剩余读档${player.loadUses}次）`)
  state._caiyueangMode = null
  endAction(state)
}
...
function deepCloneState(state) {
  return {
    players: state.players.map(p => ({
      ...
      loadUses: p.loadUses, loadMaxUses: p.loadMaxUses,
...
export function executeBetray(state) {
  ...
  addLog(state, `${player.name} 背刺 ${ally.name}！${cardDisplay(card)} +4 = ${dmg}`)
  ...
  addLog(state, `${player.name} 背刺惩罚：10回合不可结盟，防御/陷阱全明，被打伤害+2`)
